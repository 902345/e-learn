...
const verifyEmail = async (Email, Firstname, createdTeacherId) => {
    try {
        ...
        const mailOptions = {
            ...
            <br>
            <a href="${process.env.BACKEND_URL}/api/teacher/verify?id=${createdTeacherId}">
...
const addTeacherDetails = asyncHandler(async(req,res)=>{
    ...
    if (!Aadhaar || !Aadhaar.url) {
        throw new ApiError(400, "Failed to upload Aadhaar card. Please try again.")
    }
    
    const Secondary = await uploadOnCloudinary(SecondaryLocalPath)
    if (!Secondary || !Secondary.url) {
        throw new ApiError(400, "Failed to upload Secondary marksheet. Please try again.")
    }
    
    const Higher = await uploadOnCloudinary(HigherLocalPath)
    if (!Higher || !Higher.url) {
        throw new ApiError(400, "Failed to upload Higher marksheet. Please try again.")
    }
    
    const UG = await uploadOnCloudinary(UGLocalPath)
    if (!UG || !UG.url) {
        throw new ApiError(400, "Failed to upload UG marksheet. Please try again.")
    }
    
    const PG = await uploadOnCloudinary(PGLocalPath)
    if (!PG || !PG.url) {
        throw new ApiError(400, "Failed to upload PG marksheet. Please try again.")
    }
