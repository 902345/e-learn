...
function TeacherDashboard() {
  ...
  const Handlelogout = async () => {
    const response = await fetch(`${import.meta.env.VITE_BACKEND_URL}/api/teacher/logout`, {
      method: 'POST',
      credentials: "include",
      headers: {
  ...
  useEffect(() => {
    const getData = async () => {
      try {
        const response = await fetch(`${import.meta.env.VITE_BACKEND_URL}/api/Teacher/TeacherDocument/${ID}`, {
          method: 'GET',
          headers: {
            'Content-Type': 'application/json',
          },
