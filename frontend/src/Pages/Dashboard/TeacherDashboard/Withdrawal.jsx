...
function Withdrawal({ onClose, TA }) {
  ...
  const handleWithdrawl = async () => {
    if (accName === '' || accNumber === '' || ifc === '') {
      ...
      try {
        const response = await fetch(`${import.meta.env.VITE_BACKEND_URL}/api/payment/teacher/${ID}/withdraw`, {
          method: 'POST',
          headers: {
    ...
    <div className='fixed inset-0 bg-black bg-opacity-30 backdrop-blur-sm flex items-center justify-center'>
      <div className='bg-blue-600 w-80 h-96 rounded-md'>
        ...
        <div className='flex flex-col items-center justify-center mt-10 font-semibold'>
          ...
          <input type="number" placeholder='Amount' className="p-2 mb-3 rounded-md w-56 border-0 outline-0 text-gray-800" value={amount} onChange={(e) => setAmount(e.target.value)} />
