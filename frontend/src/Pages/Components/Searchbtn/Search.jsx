...
function Search() {
  ...
  const openTeacherDec = async(id,fname,lname,sub)=>{
    ...
    const data = await fetch(`${API_BASE_URL}/api/teacher/teacherdocuments`,{
        method: 'POST',
        credentials: "include",
        headers: {
  ...
  useEffect(() => {
    const getData = async () => {
      try {
        const response = await fetch(`${API_BASE_URL}/api/course/student/${ID}/enrolled`, {
          method: 'GET',
          headers: {
            'Content-Type': 'application/json',
          },
  ...
  const SearchTeacher = async (sub) => {
    ...
    const Data = await fetch(`${API_BASE_URL}/api/course/${subject}`);
  ...
  const handleEnroll = async (courseName, id) => {
    let check = await fetch(
      `${API_BASE_URL}/api/course/${courseName}/${id}/verify/student/${ID}`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
    ...
    const data = await fetch(`${API_BASE_URL}/api/payment/course/${id}/${courseName}`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
    ...
    const Key = await fetch(`${API_BASE_URL}/api/payment/razorkey`, {
      method: "GET",
      headers: {
        "Content-Type": "application/json",
      },
        ...
        if (res.statusCode === 200) {
          try {
            let response = await fetch(
              `${API_BASE_URL}/api/course/${courseName}/${id}/add/student/${ID}`,
              {
                method: "POST",
                headers: {
                  "Content-Type": "application/json",
